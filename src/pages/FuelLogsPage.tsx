...
const FuelLogsPage: React.FC = () => {
  ...
  // Calculate fuel costs for a period
  const calculateCostForPeriod = (startDate: Date, endDateExclusive: Date): number => {
    return fuelLogs
      .filter(log => {
        const logDate = new Date(log.dateTime);
        return logDate >= startDate && logDate < endDateExclusive;
      })
      .reduce((total, log) => total + (log.totalCost || 0), 0);
  };
        ...
        >
          ...
          >
            <Typography variant="h6" sx={{ mb: 3, color: theme.palette.primary.main, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
              ⛽ Fuel Cost Analysis by Time Periods
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              Total fuel costs across different time periods based on fuel log entries. 
              Costs are calculated from the totalCost field of each fuel log entry.
            </Typography>
