...
const DriverHoursPage: React.FC = () => {
  ...
  const fetchDriverHours = async () => {
    ...
    setError('');
    try {
      const res = await axios.get<DriverHour[]>(`${API_BASE}/api/driver-hours`, { params: filterProject ? { project: filterProject } : {} });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${API_BASE}/api/driver-hours/${editingId}`, {
          ...form,
        ...
        await axios.post(`${API_BASE}/api/driver-hours`, {
          ...form,
          hours: Number(form.hours),
          cost: Number(form.cost),
        });
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await axios.delete(`${API_BASE}/api/driver-hours/${deleteId}`);
