...
const WaterLogPage: React.FC = () => {
  ...
  // Calculate water costs for a period
  const calculateCostForPeriod = (startDate: Date, endDateExclusive: Date): number => {
    return waterTrips
      .filter(trip => {
        const tripDate = new Date(trip.timestamp || trip.createdAt);
        return tripDate >= startDate && tripDate < endDateExclusive;
      })
      .reduce((total, trip) => total + (trip.cost || 0), 0);
  };
        ...
        >
          ...
          >
            <Typography variant="h6" sx={{ mb: 3, color: theme.palette.primary.main, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
              💧 Water Cost Analysis by Time Periods
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              Total water costs across different time periods based on water trip entries. 
              Costs are calculated from the cost field of each water trip entry.
            </Typography>
